...
from opensora_stage2.dataset import getdataset, ae_denorm
...
from opensora_stage2.models.diffusion.diffusion import create_diffusion_T as create_diffusion
from opensora_stage2.models.diffusion.latte.modeling_latte import LatteT2V
from opensora_stage2.models.text_encoder import get_text_enc, get_text_warpper
from opensora_stage2.utils.dataset_utils import Collate
# from opensora_stage2.models.ae import ae_stride_config, ae_channel_config
from opensora_stage2.models.diffusion import Diffusion_models
from opensora_stage2.sample.pipeline_videogen import VideoGenPipeline
from opensora_stage2.utils.utils import print_grad_norm

from opensora_stage2.dataset.transform import ToTensorVideo, TemporalRandomCrop, RandomHorizontalFlipVideo, CenterCropResizeVideo, LongSideResizeVideo, SpatialStrideCropVideo
# from opensora_stage2.utils.ema import EMAModel
# from opensora_stage2.dataset import getdataset
from opensora_stage2.models import CausalVAEModelWrapper
# from opensora_stage2.models.text_encoder import get_text_enc, get_text_warpper
from opensora_stage2.models.causalvideovae import ae_stride_config, ae_channel_config
from opensora_stage2.models.causalvideovae import ae_norm, ae_denorm

# from opensora_stage2.models.diffusion import Diffusion_models, Diffusion_models_class
# from opensora_stage2.utils.dataset_utils import Collate, LengthGroupedSampler
# from opensora_stage2.sample.pipeline_opensora import OpenSoraPipeline
...
def log_validation(args, model, vae, text_encoder, tokenizer, accelerator, weight_dtype, global_step, only_sample = False):
    ...
    for validation_element in validation_data:
        ...
        validation_object_id_paths.append(validation_element['object_id_path'])
    ...
    for prompt_idx in range(len(validation_prompt)):
        ...
        object_id_path = validation_object_id_paths[prompt_idx]
    ...
    print(videos.dtype, videos.shape, "videos.dtype, videos.shape")
...
def main(args):
    ...
    for epoch in range(first_epoch, args.num_train_epochs):
        ...
        for step, (x, attn_mask, input_ids, cond_mask, cond_trajectory, cond_object_id) in enumerate(train_dataloader):
            ...
            with accelerator.accumulate(model):
                ...
                cond_object_id = cond_object_id.to(accelerator.device, dtype=weight_dtype)
                ...
                with torch.no_grad():
                    ...
                    cond_object_id = ae.encode(cond_object_id)
                ...
                cond_object_id = rearrange(cond_object_id, 'b c t h w -> b t c h w')
                cond_object_id = cond_object_id.to(torch.bfloat16)

                
                
                model_kwargs = dict(encoder_hidden_states=cond, attention_mask=attn_mask,
                                    encoder_attention_mask=cond_mask, use_image_num=args.use_image_num, cond_trajectory=cond_trajectory, cond_object_id=cond_object_id)
