...
from opensora_stage2.models.diffusion.utils.pos_embed import get_2d_sincos_pos_embed, RoPE1D, RoPE2D, LinearScalingRoPE2D, LinearScalingRoPE1D
...
class SimpleResBlock(nn.Module):
    ...
    def forward(self, x):
        ...
        out += residual
        # Final ReLU activation (optional, depending on architecture)
        out = self.relu(out)
        return out
...
class InteractionEncoder(nn.Module):
    ...
    def __init__(self):
        ...
        #self.vae = getae_wrapper(args.ae)(args.ae_path, **kwarg).to(device)
        # if args.enable_tiling:
        #     self.vae.vae.enable_tiling()
        #     self.vae.vae.tile_overlap_factor = args.tile_overlap_factor
        # convolution layer to convert l x 4 x h x w  to l x 1152 x h x w
        self.conv1 = nn.Conv2d(4, 1152 // 2, kernel_size=2, stride=2, padding=0)
        self.conv2 = nn.Conv2d(4, 1152 // 2, kernel_size=2, stride=2, padding=0)
        # resblock that takes in l x 1152 x d and outputs l x 1152 x d
        self.resblock1 = SimpleResBlock(1152)
        self.resblock2 = SimpleResBlock(1152)
        self.N = 1


    def forward(
            self,
            trajectory: torch.Tensor, # shape: (B, T, C, H, W)
            object_id: torch.Tensor, # shape: (B, T, C, H, W)
    ):
        b, t, c, h, w  = trajectory.shape
        print(object_id.shape, "object_id.shape")
        assert trajectory.shape == object_id.shape
        # latents = trajectory.permute(0,2,1,3,4)
        # latents = self.vae.encode(trajectory) # (B, 4, T/4, H/8, W/8)
        # b, c, t, h, w = latents.shape

        motion_patches_trajectory = self.conv1(trajectory.view(b*t, c, h, w))
        motion_patches_object_id = self.conv2(object_id.view(b*t, c, h, w))
        motion_patches = torch.cat([motion_patches_trajectory, motion_patches_object_id], dim=1)

        bt, s, h, w = motion_patches.shape
        # motion_patches = motion_patches.view(b, t, s, h*w)
        # Residual blocks
        for i in range(self.N):
            motion_patches = self.resblock1(motion_patches)
            motion_patches = self.resblock2(motion_patches)
            # identity = motion_patches
            # motion_patches = self.resblock1(motion_patches)
            # identity = motion_patches + identity
            # motion_patches = self.resblock2(identity)
            # motion_patches = motion_patches + identity
        
        motion_patches = motion_patches.view(b, t, s, h*w)
        motion_patches = motion_patches.permute(0, 3, 1, 2)
...
class BasicTransformerBlock(nn.Module):
    ...
    def forward(
        ...
        if self.use_ada_layer_norm:
            ...
            print("self.use_layer_norm")
            norm_hidden_states = self.norm1(hidden_states)
        elif self.use_ada_layer_norm_single: # this one
            ...
            norm_hidden_states = norm_hidden_states * (1 + scale_msa) + shift_msa
            # norm_hidden_states.shape: torch.Size([21, 1024, 1152])
            norm_hidden_states = norm_hidden_states.squeeze(1)
            # norm_hidden_states.shape: torch.Size([21, 1024, 1152])
        else:
            raise ValueError("Incorrect norm used")
...
class AdaLayerNormSingle(nn.Module):
    ...
    Parameters:
        embedding_dim (`int`): The size of each embedding vector. 1152
        use_additional_conditions (`bool`): To use additional conditions for normalization or not. False
    """
