...
from opensora_stage2.models.diffusion.utils.pos_embed import get_1d_sincos_pos_embed, PositionGetter1D, PositionGetter2D
from opensora_stage2.models.diffusion.latte.modules import PatchEmbed, BasicTransformerBlock, BasicTransformerBlock_, AdaLayerNormSingle, \
    Transformer3DModelOutput, CaptionProjection, InteractionEncoder, InteractionAttention
...
class LatteT2V(ModelMixin, ConfigMixin):
    ...
    def __init__(
        ...
        # print("\n\n\n\n\n\n===================================================")
        # print("height, width, patch_size, in_channels, embed_dim, interpolation_scale: ", sample_size[0], sample_size[1], patch_size, in_channels, inner_dim, interpolation_scale_2d)
        # print("===================================================\n\n\n\n\n\n")
        self.pos_embed = PatchEmbed(
            height=sample_size[0], # 64
            width=sample_size[1], # 64
            patch_size=patch_size, # 2
            in_channels=in_channels, # 4
            embed_dim=inner_dim, # 1152
            interpolation_scale=interpolation_scale_2d, # 1
        )
        ...
        self.interaction_attentions = nn.ModuleList(
            [
        ...
        if norm_type == "ada_norm_single":
            ...
            # print("\n\n\n\n\n\n===================================================")
            # print("inner_dim, self.use_additional_conditions", inner_dim, self.use_additional_conditions)
            # print("===================================================\n\n\n\n\n\n")
            # inner_dim, self.use_additional_conditions: 1152 False
    ...
    def forward(
        ...
        #cond_trajectory = torch.zeros([1,16,4,64,64], device=hidden_states.device).to(torch.bfloat16)
        
        
        interaction_condition = self.interaction_encoder(cond_trajectory, cond_object_id)
        te_b, te_hw, te_t, te_d = interaction_condition.shape
        interaction_condition = interaction_condition.reshape(te_b * te_hw, te_t, te_d)


        
        # pirnt("\n\n\n$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$", interaction_condition.shape, "$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$\n\n\n")
        ...
        #print("\n\n\n\n--------------------------------")
        #print("attention_mask.shape, attention_mask_compress.shape", attention_mask.shape, attention_mask_compress.shape)
        #attention_mask.shape, attention_mask_compress.shape torch.Size([21, 1, 1024]) torch.Size([21, 1, 1024])
        #print("--------------------------------\n\n\n\n")
        ...
        if self.is_input_patches:  # here
            ...
            #print("before self.pos_embed, hidden_states.shape", hidden_states.shape)

            # the dtype is changed here
            hidden_states = self.pos_embed(hidden_states.to(self.dtype))  # alrady add positional embeddings
            #print("after self.pos_embed, hidden_states.shape", hidden_states.shape)
            #before self.pos_embed, hidden_states.shape torch.Size([21, 4, 64, 64])
            #after self.pos_embed, hidden_states.shape torch.Size([21, 1024, 1152])
            ...
            if self.adaln_single is not None:
                ...
                # timestep.shape, timesteps_proj.shape:  torch.Size([1, 6912]) torch.Size([1, 1152])
        # 2. Blocks
        ...
        if self.caption_projection is not None:
            ...
            #  before self.caption_projection, encoder_hidden_states.shape torch.Size([1, 5, 300, 4096])
            #  after self.caption_projection, encoder_hidden_states.shape torch.Size([1, 5, 300, 1152])
        ...
        timestep_spatial_no_images = repeat(timestep, 'b d -> (b f) d', f=frame).contiguous()
        timestep_temp = repeat(timestep, 'b d -> (b p) d', p=num_patches).contiguous()
        
        # timestep_spatial.shape, timestep_temp.shape: torch.Size([21, 6912]) torch.Size([1024, 6912])
        ...
        for i, (spatial_block, temp_block, interaction_attention) in enumerate(zip(self.transformer_blocks, self.temporal_transformer_blocks, self.interaction_attentions)):
        #for i, (spatial_block, temp_block, interaction_attention) in enumerate(zip(self.transformer_blocks, self.temporal_transformer_blocks, self.interaction_attentions)):
            # self.gradient_checkpointing, self.training True True
            if self.training and self.gradient_checkpointing:
                
                
                # before, hidden_states.shape torch.Size([21, 1024, 1152])
                # torch.utils.checkpoint.checkpoint is used to save memory
                
                hidden_states = torch.utils.checkpoint.checkpoint(
                    spatial_block,
                    hidden_states,
                    attention_mask_compress if i >= self.num_layers // 2 else attention_mask,  # all 0, I think this has not been used
                ...
                # after, hidden_states.shape torch.Size([21, 1024, 1152])
                
               
                # if i >= self.num_layers // 2:
                # attention_mask_compress.shap torch.Size([21, 1, 1024])
                # else:
                # attention_mask.shape torch.Size([21, 1, 1024])
                ...
                if enable_temporal_attentions:
                    ...
                    if use_image_num != 0:  # image-video joitn training
                        ...
                        #interaction_condition = interaction_condition.permute(0, 2, 1, 3)
                        ...
                        hidden_states_video = torch.utils.checkpoint.checkpoint(
                            interaction_attention,
                            hidden_states_video,
                            None,  # all 0, I think this has not been used
                            interaction_condition,
                            None,
                            timestep_temp,
                            cross_attention_kwargs,
                ...
                if enable_temporal_attentions:
                    ...
                    if use_image_num != 0 and self.training:
                        ...
                        hidden_states_video = interaction_attention(
                            hidden_states_video,
                            None,  # all 0, I think this has not been used
                            interaction_condition,
                            None,
                    ...
                    # 99
                        ...
                        hidden_states = interaction_attention(
                            hidden_states,
                            None,  # all 0, I think this has not been used
                            interaction_condition,
                            None,
...
if __name__ == '__main__':
    from opensora_stage2.models.ae import ae_channel_config, ae_stride_config
    from opensora_stage2.models.ae import getae, getae_wrapper
    from opensora_stage2.models.ae.videobase import CausalVQVAEModelWrapper, CausalVAEModelWrapper
