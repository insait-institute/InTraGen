...
#             print(batch[0]['video_data'].keys(), "batch[0]['video_data'].keys() \n\n\n")
#             cond_trajectory = torch.stack([i['video_data']['trajectory_anno'] for i in batch])  # b 64 32 32
...
#         return batch_tubes_vid, input_ids_vid, cond_mask_vid, batch_tubes_img, input_ids_img, cond_mask_img, cond_trajectory

#     def __call__(self, batch):
#         batch_tubes_vid, input_ids_vid, cond_mask_vid, batch_tubes_img, input_ids_img, cond_mask_img, cond_trajectory = self.package(batch)
...
#         return pad_batch_tubes, attention_mask, input_ids, cond_mask, cond_trajectory
...
class Collate:
    ...
    def package(self, batch):
        ...
        if self.num_frames > 1:
            ...
            input_ids_vid = torch.stack([i['video_data']['input_ids'] for i in batch])  # b 1 l
            cond_mask_vid = torch.stack([i['video_data']['cond_mask'] for i in batch])  # b 1 l
            cond_trajectory = torch.stack([i['video_data']['trajectory_anno'] for i in batch])  # b [c t h w]
            cond_object_id = torch.stack([i['video_data']['object_id'] for i in batch])  # b [c t h w]
            
        if self.num_frames == 1 or self.use_image_num != 0: 
            batch_tubes_img = [j for i in batch for j in i['image_data']['image']]  # b*num_img [c 1 h w]
            input_ids_img = torch.stack([i['image_data']['input_ids'] for i in batch])  # b image_num l
            cond_mask_img = torch.stack([i['image_data']['cond_mask'] for i in batch])  # b image_num l
        return batch_tubes_vid, input_ids_vid, cond_mask_vid, batch_tubes_img, input_ids_img, cond_mask_img, cond_trajectory, cond_object_id

    def __call__(self, batch):
        batch_tubes_vid, input_ids_vid, cond_mask_vid, batch_tubes_img, input_ids_img, cond_mask_img, cond_trajectory, cond_object_id = self.package(batch)
        ...
        if self.num_frames > 1 and self.use_image_num == 0:
            ...
            attention_mask = rearrange(attention_mask_img, '(b i) 1 h w -> b i h w', i=1)
            input_ids, cond_mask = input_ids_img, cond_mask_img  # b 1 l
        return pad_batch_tubes, attention_mask, input_ids, cond_mask, cond_trajectory, cond_object_id
