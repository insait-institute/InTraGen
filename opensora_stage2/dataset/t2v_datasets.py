...
#             print("\n\n\n\n\n\ndict(video_data=video_data, image_data=image_data)\n\n\n\n\n\n")
...
#         anno_path = self.vid_cap_list[idx]['anno_path']
#         #video_anno_path = self.vid_cap_list[idx]['anno_path']
#         print("\n\n\n===========================")
#         print(anno_path)
#         trajectory_anno = np.load(anno_path)
#         print(trajectory_anno.shape)
#         print("===========================\n\n\n")
#         # trajectory_anno = trajectory_anno[0,:,:,:,:]
#         # trajectory_anno = trajectory_anno.transpose(1, 0, 2, 3)
#         #trajectory_anno = None

#         # frame_idx = self.vid_cap_list[idx]['frame_idx']
#         frame_idx = None
#         video, trajectory_anno = self.decord_read(video_path, trajectory_anno, frame_idx)




#         trajectory_anno = torch.from_numpy(trajectory_anno).to(torch.bfloat16)
...
#         #video_anno = np.load(video_anno_path)
...
#         return dict(video=video, input_ids=input_ids, cond_mask=cond_mask, trajectory_anno=trajectory_anno)
...
#     def decord_read(self, path, trajectory_anno, frame_idx=None):
...
#         frame_indice = np.linspace(start_frame_ind, end_frame_ind - 1, self.num_frames, dtype=int)
#         # frame_indice = np.linspace(0, 63, self.num_frames, dtype=int)
#         trajectory_anno = trajectory_anno[start_frame_ind // 4:(end_frame_ind-1) // 4]
        
#         video_data = decord_vr.get_batch(frame_indice).asnumpy()
#         video_data = torch.from_numpy(video_data)
#         #video_data = video_data.permute(0, 1, 3, 2)  # (T, H, W, C) -> (T C H W)
        
#         return video_data, trajectory_anno


#     def decord_read_try(self, path, trajectory_anno, frame_idx=None):
#         decord_vr = self.v_decoder(path)
#         total_frames = len(decord_vr)
#         # Sampling video frames
#         if frame_idx is None:
...
from opensora_stage2.utils.utils import text_preprocessing
...
class T2V_dataset(Dataset):
    ...
    def get_video(self, idx):
        ...
        video_path = self.vid_cap_list[idx]['path']
        trajectory_anno_path = self.vid_cap_list[idx]['sparse_pose_path']

        object_id_path = self.vid_cap_list[idx]['object_id_path']
        
        # trajectory_anno = np.load(video_anno_path)
        # trajectory_anno = trajectory_anno[0,:,:,:,:]
        # trajectory_anno = trajectory_anno.transpose(1, 0, 2, 3)

        # frame_idx = self.vid_cap_list[idx]['frame_idx']
        frame_idx = None
        video, trajectory_anno, object_id = self.decord_read(video_path, trajectory_anno_path, object_id_path, frame_idx)
        ...
        input_ids = text_tokens_and_mask['input_ids']
        cond_mask = text_tokens_and_mask['attention_mask']
        return dict(video=video, input_ids=input_ids, cond_mask=cond_mask, trajectory_anno=trajectory_anno, object_id=object_id)
    ...
    def decord_read(self, path, trajectory_anno_path, object_id_path, frame_idx=None):    
        decord_vr = self.v_decoder(path)
        decord_vr_trajectory = self.v_decoder(trajectory_anno_path)
        ...
        assert total_frames == total_frames_trajectory == total_frames_object_id
        ...
        trajectory_anno = decord_vr_trajectory.get_batch(frame_indice).asnumpy()
        trajectory_anno = torch.from_numpy(trajectory_anno)
        trajectory_anno = trajectory_anno.permute(0, 3, 1, 2)

        object_id = decord_vr_object_id.get_batch(frame_indice).asnumpy()
        object_id = torch.from_numpy(object_id)
        object_id = object_id.permute(0, 3, 1, 2)
        
        return video_data, trajectory_anno, object_id



    def get_vid_cap_list(self):
        if self.video_data.split('.')[-1] == 'json':
            with open(self.video_data, 'r') as f:
                vid_cap_list = json.load(f)
            return vid_cap_list
